...
export PROJECT_VERSION=3.11.0-SNAPSHOT
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=3.11.0-SNAPSHOT
...
if [ -z "3.11.0-SNAPSHOT" ]
then
  echo "Enforce version not set, skipping"
else
  echo "Setting version to 3.11.0-SNAPSHOT"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="3.11.0-SNAPSHOT"  | tee /root/project/logs/enforce-version.log
fi
